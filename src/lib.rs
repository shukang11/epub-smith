...
/// 从章节标题中提取序号
pub fn extract_chapter_number(title: &str, extraction_rules: &Option<crate::models::ChapterNumberExtraction>) -> Option<usize> {
    use regex::Regex;
    ...
    for rule in rules {
        if let Ok(regex) = Regex::new(&rule.pattern) {
            if let Some(captures) = regex.captures(title) {
                if let Some(num_str) = captures.get(rule.capture_group) {
                    if let Some(num) = convert_number(num_str.as_str(), &rule.number_type) {
                        return Some(num);
                    }
                }
            }
