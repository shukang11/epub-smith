...
fn test_custom_extraction_rules() {
    ...
    // 测试自定义规则
    assert_eq!(extract_chapter_number("Episode 1: The Beginning", &Some(custom_rules.clone())), Some(1));
    assert_eq!(extract_chapter_number("Episode 2: The Journey", &Some(custom_rules.clone())), Some(2));
    assert_eq!(extract_chapter_number("Special 1: Bonus Content", &Some(custom_rules.clone())), Some(1));
    
    // 测试自定义规则无法匹配的情况
    assert_eq!(extract_chapter_number("Chapter 1", &Some(custom_rules.clone())), None);
    assert_eq!(extract_chapter_number("第1章", &Some(custom_rules.clone())), None);
}
...
fn test_mixed_chapter_formats() {
    ...
    assert_eq!(extract_chapter_number("Chapter 1", &extraction_rules), Some(1));
    assert_eq!(extract_chapter_number("Chapter I", &extraction_rules), Some(1));
    assert_eq!(extract_chapter_number("Section 1", &extraction_rules), Some(1));
