...
| `cover.rs` | 用书名/作者填充 SVG 封面模板，经 resvg 栅格化为 PNG | 仅处理封面生成，不参与渲染和打包 |
...
| `export.rs` | 导出样式模板与封面模板（cover.svg）到指定目录 | 仅处理模板导出功能，不涉及其他业务逻辑 |
...
| 文件 | 作用 | 边界 |
|------|------|------|
| `renderer/mod.rs` | 将Book数据渲染为XHTML，处理封面（自动生成/复制）与封面页 | 仅处理XHTML渲染与封面，不涉及EPUB打包 |
...
| `templates/nav.xhtml.tera` | 导航XHTML模板（`.container` 包裹 `#toc`，章节链接补零） | 仅定义模板结构，不包含业务逻辑 |
| `templates/toc.ncx.tera` | EPUB2 兼容的NCX导航模板（navMap 一层结构） | 仅定义模板结构，不包含业务逻辑 |
